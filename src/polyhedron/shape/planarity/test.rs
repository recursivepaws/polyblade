...
fn test_simple_planar_graph() {
    let graph = Distance::from_edges(vec![
        [1, 2],
        [2, 3],
        [3, 4],
        [4, 6],
...
fn test_multiple_components_planar_graph() {
    let graph = Distance::from_edges(vec![[1, 2], [2, 3], [3, 1], [4, 5], [5, 6], [6, 4]]);
    assert!(graph.is_planar())
}

#[test]
fn test_planar_multi_graph() {
    let graph = Distance::from_edges(vec![[0, 1], [0, 1], [0, 1], [1, 2], [2, 0]]);
    assert!(graph.is_planar())
}
...
impl Distance {
    pub fn from_edges(edges: Vec<[VertexId; 2]>) -> Self {
        let mut vertices: Vec<usize> = edges.iter().flat_map(|edge| edge.to_vec()).collect();
        vertices.dedup();
        let order = vertices.len();

        let mut dist = Distance::new(order);
        for edge in edges {
            dist.connect(edge);
        }
        dist
